...
public class Dog implements Pet {
    ...
    public Sayable toSayable(final int i) {
        class DogPet extends SuperDog implements Sayable{
            ...
            public String say()
            {
                if (i <= 0) return getSuperQuotes() + getName() + getSuperQuotes() + " спит.";
                else {
                    StringBuilder str = new StringBuilder("г");
                    for (int j = 0; j < i; j++) str.append("а");
                    str.append("в! ");
                    return getSuperQuotes() + getName() + getSuperQuotes() + " лает " + str + formatter.format(new Date());
                }
            }
