...
public class Solution {
    ...
    public static class AmigoThreadFactory implements ThreadFactory{
        static AtomicInteger atomicFabric = new AtomicInteger(0);
        AtomicInteger atomicThread = new AtomicInteger(0);
        ThreadGroup group = Thread.currentThread().getThreadGroup();

        AmigoThreadFactory(){
            atomicFabric.incrementAndGet();
        }
        ...
        public Thread newThread(Runnable r)
        {
            synchronized (this)
            {
                ...
                thread.setName(String.format("%s-pool-%d-thread-%d", group.getName(),
                        atomicFabric.get(), atomicThread.incrementAndGet()));
                return thread;
            }
